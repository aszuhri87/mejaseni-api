        ...
        var _componentPage = function(){
            ...
            const initTable = () => {
                init_table = $('#init-table').DataTable({
                    ...
                    ajax: {
                        type: 'POST',
                        url: "{{ url('admin/cms/marketplace/dt') }}",
                    },
                    columns: [
                        { data: 'DT_RowIndex' },
                        { data: 'image_url' },
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render : function(data, type, full, meta) {
                                return `
                                    <a href="{{url('/admin/cms/marketplace/update')}}/${data}" title="Edit" class="btn btn-edit btn-sm btn-clean btn-icon mr-2" title="Edit details">
                                    ...
                                    <a href="{{url('/admin/cms/marketplace')}}/${data}" title="Delete" class="btn btn-delete btn-sm btn-clean btn-icon" title="Delete">
                                        <span class="svg-icon svg-icon-md">
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                    <rect x="0" y="0" width="24" height="24"/>
            ...
            initAction = () => {
                $(document).on('click', '#add-btn', function(event){
                    ...
                    $('#form-marketplace').trigger("reset");
                    $('#form-marketplace').attr('action','{{url('admin/cms/marketplace')}}');
                    $('#form-marketplace').attr('method','POST');

                    $('#image').html('<input type="file" name="image" class="dropify image"/>');
                    $('.dropify').dropify();

                    showModal('modal-marketplace');
                });
                ...
                $(document).on('click', '.btn-edit', function(event){
                    ...
                    $('#form-marketplace').trigger("reset");
                    $('#form-marketplace').attr('action', $(this).attr('href'));
                    $('#form-marketplace').attr('method','POST');

                    $('#form-marketplace').find('input[name="name"]').val(data.name);
                    $('#form-marketplace').find('textarea[name="url"]').val(data.url);
                    $('#image').empty();
                ...
                $(document).on('click', '.btn-delete', function(event){
                    ...
                    Swal.fire({
                        ...
                        text: "Deleted Market Place will be permanently lost!",
                        icon: 'warning',
