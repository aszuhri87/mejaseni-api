        ...
        var _componentPage = function(){
                ...
                init_table = $('#init-table').DataTable({
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render : function(data, type, full, meta) {
                                return `
                                    <a href="{{url('/admin/cms/galery/update')}}/${data}" title="Edit" class="btn btn-edit btn-sm btn-clean btn-icon mr-2" title="Edit details">
                                    ...
                                    <a href="{{url('/admin/cms/galery')}}/${data}" title="Delete" class="btn btn-delete btn-sm btn-clean btn-icon" title="Delete">
                                        <span class="svg-icon svg-icon-md">
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                    <rect x="0" y="0" width="24" height="24"/>
            ...
            initAction = () => {
                $(document).on('click', '#add-btn', function(event){
                    ...
                    $('#form-galery').trigger("reset");
                    $('#form-galery').attr('action','{{url('admin/cms/galery')}}');
                    $('#form-galery').attr('method','POST');

                    $('#image').html('<input type="file" name="image" class="dropify image"/>');
                    $('.dropify').dropify();

                    showModal('modal-galery');
                });
                ...
                $(document).on('click', '.btn-edit', function(event){
                    ...
                    $('#form-galery').trigger("reset");
                    $('#form-galery').attr('action', $(this).attr('href'));
                    $('#form-galery').attr('method','POST');

                    $('#form-galery').find('input[name="title"]').val(data.title);
                    $('#form-galery').find('input[name="date"]').val(data.date);
                    $('#form-galery').find('textarea[name="description"]').val(data.description);
                    $('#image').empty();
                ...
                $(document).on('click', '.btn-delete', function(event){
                    ...
                    Swal.fire({
                        title: 'Delete Galery?',
                        text: "Deleted Galery will be permanently lost!",
                        icon: 'warning',
