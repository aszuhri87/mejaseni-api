        ...
        var _componentPage = function(){
            ...
            const initTable = () => {
                init_table = $('#init-table').DataTable({
                    ...
                    ajax: {
                        type: 'POST',
                        url: "{{ url('admin/cms/fun-creative/dt') }}",
                    },
                    columns: [
                        { data: 'DT_RowIndex' },
                        { data: 'image_url' },
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render : function(data, type, full, meta) {
                                return `
                                    <a href="{{url('/admin/cms/fun-creative/update')}}/${data}" title="Edit" class="btn btn-edit btn-sm btn-clean btn-icon mr-2" title="Edit details">
                                    ...
                                    <a href="{{url('/admin/cms/fun-creative')}}/${data}" title="Delete" class="btn btn-delete btn-sm btn-clean btn-icon" title="Delete">
                                        <span class="svg-icon svg-icon-md">
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                    <rect x="0" y="0" width="24" height="24"/>
            ...
            initAction = () => {
                $(document).on('click', '#add-btn', function(event){
                    ...
                    $('#form-fun-creative').trigger("reset");
                    $('#form-fun-creative').attr('action','{{url('admin/cms/fun-creative')}}');
                    $('#form-fun-creative').attr('method','POST');

                    $('#image').html('<input type="file" name="image" class="dropify image"/>');
                    $('.dropify').dropify();

                    showModal('modal-fun-creative');
                });
                ...
                $(document).on('click', '.btn-edit', function(event){
                    ...
                    $('#form-fun-creative').trigger("reset");
                    $('#form-fun-creative').attr('action', $(this).attr('href'));
                    $('#form-fun-creative').attr('method','POST');
            ...
            formSubmit = () => {
                $('#form-fun-creative').submit(function(event){
                    event.preventDefault();
