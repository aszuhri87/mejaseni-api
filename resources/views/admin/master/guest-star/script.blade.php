<script type="text/javascript">
    var Page = function() {
        var _componentPage = function(){
            var init_table, docsDropzone, init_expertise, init_coach;
            var arr_path = [];
            ...
            const initTable = () => {
                init_table = $('#init-table').DataTable({
                    ...
                    ajax: {
                        type: 'POST',
                        url: "{{ url('admin/master/guest-star/dt') }}",
                    },
                    columns: [
                        { data: 'DT_RowIndex' },
                        { data: 'name' },
            ...
            initAction = () => {
                $(document).on('click', '#add-btn', function(event){
                    ...
                    $('#form-guest-star').trigger("reset");
                    $('#form-guest-star').attr('action','{{url('admin/master/guest-star')}}');
                    $('#form-guest-star').attr('method','POST');

                    $('#switch-coach').attr('checked', false)

                    $('.coach-place').hide();
                    $('.guest-place').show();

                    $('#i-description').prop('required', true);
                    $('#i-name').prop('required', true);

                    $('#i-file-upload').val('')
                    $('#image-preview').prop('src', "{{ url('/assets/images/profile.png') }}")

                    get_expertise();

                    showModal('modal-guest-star');
                });
                ...
                $(document).on('click', '.btn-edit', function(event){
                    ...
                    $('#form-guest-star').trigger("reset");
                    $('#form-guest-star').attr('action', $(this).attr('href'));
                    $('#form-guest-star').attr('method','PUT');

                    if(data.coach_id){
                        $('#switch-coach').attr('checked', true)

                        $('.guest-place').hide();
                        $('.coach-place').show();

                        $('#i-description').prop('required', false);
                        ...
                        $('#form-guest-star').find('input[name="name"]').val(data.name);
                        $('#form-guest-star').find('textarea[name="description"]').val(data.description);
                        $('#i-file-upload').val(data.image)
                        $('#image-preview').prop('src',data.image_url)
                ...
                $(document).on('change', '#switch-coach', function(){
                ...
                $(document).on('change', '#file-upload', function(){
                    ...
                    .done(function(res, xhr, meta) {
                        $('#image-preview').prop('src', res.url)
                        $('#i-file-upload').val(res.data.path)
                    })
                    .fail(function(res, error) {
                        toastr.error(res.responseJSON.message, 'Failed')
                    });
