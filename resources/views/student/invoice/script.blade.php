        ...
        var _componentPage = function(){
                ...
                init_table = $('#init-table').DataTable({
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render : function(data, type, full, meta) {
                                return `
                                    <a href="javascript:void(0)" title="Lihat Detail" class="btn btn-detail btn-outline-primary btn-sm mr-2">
            ...
            initAction = () => {
                $(document).on('click','.btn-detail',function(event){
                    ...
                    let data = init_table.row($(this).parents('tr')).data();
                    if(data.classroom_id){
                        $('#class').html(data.classroom_name);
                    }
                    else if(data.master_lesson_id){
                        $('#class').html(data.master_lesson_name);
                    }
                    else if(data.theory_id){
                        $('#class').html(data.theory_name);
                    }else{
                        $('#class').html(data.session_video_name);
                    }

                    $('#sub-total').html(`Rp. ${numeral(data.total).format('0,0')}`);
                    $('#total').html(`Rp. ${numeral(data.total).format('0,0')}`);
